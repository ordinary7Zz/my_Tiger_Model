...
def create_prompt(subtype: str) -> str:
    subtype_upper = subtype.upper()
    if "PTC" in subtype_upper:
        return "papillary thyroid carcinoma ultrasound, malignant nodule, irregular margin"
    if "FTC" in subtype_upper:
        return "follicular thyroid carcinoma ultrasound, malignant nodule, well circumscribed"
    return "thyroid nodule ultrasound, realistic lesion"
...
def augment_train_with_tiger(
    ...
    for sample in train_samples:
        ...
        prompt = create_prompt(sample.subtype)
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="JSON 驱动版 run_tiger_model（完整版）")
